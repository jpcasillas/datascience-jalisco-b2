...
gr <- ggplot(tempo, aes(x=inegi, y=total)) +
  ...
  geom_text(aes(x=inegi, y=total, label=paste(nom_mun, nom_ent, total, sep=", ")), data=tempo[tempo$total>100,], hjust=0, vjust=-1, size=3) +
...
gr <- ggplot(tempo, aes(x=des12a17, y=des26a40, color=sexo)) +
  ...
  labs(title="Porcentaje de desaparecidos por sexo \n 12 a 17 años vs 26 a 40 años",
       x="% de desaparecidos 26 a 40 años", y="% de desaparecidos 12 a 17 años", color="Sexo") +
  theme_bw()
