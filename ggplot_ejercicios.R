...
gr <- ggplot(tempo, aes(x=sexo, y=promedio)) +
  ...
  geom_text(aes(label=round(promedio, 2)),vjust=0) +
  labs(title="Tasa de desaparecidos 2011", subtitle = "Por sexo", x="Sexo", y="Promedio")
...
gr <- ggplot(tempo, aes(x=sexo, y=porcentaje, fill=rango_edad)) +
  ...
  geom_text(aes(label=paste0(porcentaje,"%")), position=position_stack(vjust=0.5), angle=1) +
